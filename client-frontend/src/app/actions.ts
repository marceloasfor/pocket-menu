...
export async function getAllUsers(verification_code:string|null) {
    const res = await fetch(`http://localhost:8000/tables/member?verification_code=${verification_code}`, {
        method: 'GET'
    });
...
export async function anonLogin(username:string, verification_code:string|null) {
    ...
    const res = await fetch('http://localhost:8000/tables/member/', {
        method: 'POST',
...
export async function exitTable(username:string) {
    ...
    const res = await fetch('http://localhost:3000/tables/member/', {
        method: 'DELETE',
...
export async function getAllOrders(verification_code:string|null) {
    const res = await fetch(`http://localhost:8000/order?verification_code=${verification_code}`, {
        method: 'GET'
    });
    const result = await res.json();
    return result;
